...
01 70 54 71 83 51 54 69 16 92 33 48 61 43 52 01 89 19 67 48"""
  ...
  def reformatNumStr(self):
    return [[int(s) for s in row.split(' ')] for row in self.numStr.split('\n')]

  def searchGrid(self):
    grid = self.reformatNumStr()
    max = 0
    # horizontal search
    for row in grid:
      for col in range(len(row) - 3):
        prod = row[col] * row[col + 1] * row[col + 2] * row[col + 3]
        if prod > max:
          max = prod
    # vertical search
...
test = PE011()
# print(test.reformatNumStr())
print(test.searchGrid())  # 70600674
